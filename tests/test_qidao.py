...
def test_get_vaul_address():
    address = QiDao.get_vault_address(GnosisTokenAddr.WETH, Chain.XDAI)
    assert address == "0x5c49b268c9841AFF1Cc3B0a418ff5c3442eE3F3b"


def test_get_vault_data():
    block = 27814350
    data = QiDao.get_vault_data(0, GnosisTokenAddr.GNO, block, Chain.XDAI)
...
def test_underlying():
    block = 27814350
    underlying = QiDao.underlying(0, GnosisTokenAddr.GNO, block, Chain.XDAI)
