...
from defyes.constants import GNO_XDAI, XDAI
...
def test_get_all_rewards(campaigns, db):
    x = Swapr.get_all_rewards(
        TEST_WALLET, DXS, TEST_BLOCK, XDAI, WEB3, decimals=True, campaigns=campaigns, distribution_contracts=None, db=db
    )
...
def test_underlying(campaigns, db, decimals, reward):
    x = Swapr.underlying(
        TEST_WALLET, DXS, TEST_BLOCK, XDAI, WEB3, decimals=decimals, reward=reward, campaigns=campaigns, db=db
    )
