...
def test_get_all_rewards(decimals):
    rewards = Curve.get_all_rewards(TEST_WALLET, X3CRV_ETH, TEST_BLOCK, ETHEREUM, web3=WEB3, decimals=decimals)
...
def test_underlying(reward, decimals):
    u = Curve.underlying(TEST_WALLET, X3CRV_ETH, TEST_BLOCK, ETHEREUM, web3=WEB3, reward=reward, decimals=decimals)
    expected = [
...
def test_unwrap(lptoken_amount, decimals):
    u = Curve.unwrap(lptoken_amount, X3CRV_ETH, TEST_BLOCK, ETHEREUM, web3=WEB3, decimals=decimals)
    expected = {
...
def test_swap_fees(decimals):
    sf = Curve.swap_fees(X3CRV_ETH, TEST_BLOCK - 100, TEST_BLOCK, ETHEREUM, web3=WEB3, decimals=decimals)
