...
from defyes.constants import CRV_ETH, DAI_ETH, Chain, USDC_ETH, USDT_ETH, X3CRV_ETH
...
def test_get_lptoken_data():
    lpt_data = Curve.get_lptoken_data(X3CRV_ETH, TEST_BLOCK, Chain.ETHEREUM, web3=WEB3)
...
def test_get_pool_address():
    ...
    pa = Curve.get_pool_address(WEB3, X3CRV_ETH, TEST_BLOCK, Chain.ETHEREUM)
    assert pa == CURVE_3POOL


def test_get_pool_gauge_address():
    pga = Curve.get_pool_gauge_address(WEB3, CURVE_3POOL, X3CRV_ETH, TEST_BLOCK, Chain.ETHEREUM)
...
def test_get_pool_data():
    ...
    expected = {"is_metapool": False, "coins": {0: DAI_ETH, 1: USDC_ETH, 2: USDT_ETH}}
    assert expected == {k: pd[k] for k in expected}


@pytest.mark.parametrize("decimals", [True, False])
def test_get_all_rewards(decimals):
    rewards = Curve.get_all_rewards(TEST_WALLET, X3CRV_ETH, TEST_BLOCK, Chain.ETHEREUM, web3=WEB3, decimals=decimals)
...
def test_underlying(reward, decimals):
    u = Curve.underlying(TEST_WALLET, X3CRV_ETH, TEST_BLOCK, Chain.ETHEREUM, web3=WEB3, reward=reward, decimals=decimals)
    expected = [
        [DAI_ETH, Decimal("0"), Decimal("0")],
...
def test_unwrap(lptoken_amount, decimals):
    u = Curve.unwrap(lptoken_amount, X3CRV_ETH, TEST_BLOCK, Chain.ETHEREUM, web3=WEB3, decimals=decimals)
    expected = {
        0: [[DAI_ETH, Decimal("0")], [USDC_ETH, Decimal("0")], [USDT_ETH, Decimal("0")]],
        10: [
...
def test_pool_balances(decimals):
    pb = Curve.pool_balances(X3CRV_ETH, TEST_BLOCK, Chain.ETHEREUM, web3=WEB3, decimals=decimals)
    assert pb == [
        [DAI_ETH, Decimal("165857824629254122209119338") / Decimal(10**18 if decimals else 1)],
...
def test_swap_fees(decimals):
    sf = Curve.swap_fees(X3CRV_ETH, TEST_BLOCK - 100, TEST_BLOCK, Chain.ETHEREUM, web3=WEB3, decimals=decimals)
...
def test_get_base_apr(apy):
    # x = Curve.get_base_apr(X3CRV_ETH, Chain.ETHEREUM, TEST_BLOCK, web3=WEB3, apy=apy)
    assert True
...
def test_swap_fees_v2(apy):
    # sf = Curve.swap_fees_v2(X3CRV_ETH, Chain.ETHEREUM, TEST_BLOCK, web3=WEB3, apy=apy)
    assert True


@pytest.mark.skip("web3.exceptions.ABIFunctionNotFound")
def test_get_swap_fees_APR():
    # sf = Curve.get_swap_fees_APR(X3CRV_ETH, Chain.ETHEREUM, TEST_BLOCK, web3=WEB3)
    assert True
