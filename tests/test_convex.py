...
def test_get_rewards():
    ...
    rw_contract = get_contract(rewarders[0], ETHEREUM,
                               web3=web3, abi=Convex.ABI_REWARDS,
                               block=16993460)
    wallet = '0x58e6c7ab55Aa9012eAccA16d1ED4c15795669E1C'
    rewards = Convex.get_rewards(web3, rw_contract, wallet, 16993460,
...
def test_get_extra_rewards(lp_token, wallet):
    ...
    rw_contract = get_contract(rewarders[0], ETHEREUM,
                               web3=web3, abi=Convex.ABI_REWARDS,
                               block=16993460)
    extra_rewards = Convex.get_extra_rewards(web3, rw_contract, wallet, 16993460,
                                            ETHEREUM, decimals=False)
    assert extra_rewards == [[LDO_ETH, Decimal('1680694318843318519229')]]


def test_get_cvx_mint_amount():
    cvx_mint_amount = Convex.get_cvx_mint_amount(web3, Decimal('6649.47123882958317496'), 17499865, ETHEREUM, decimals=False)
    assert cvx_mint_amount == [CVX_ETH, Decimal('106.9162438469377937584223851')]


def test_get_all_rewards():
    all_rewards = Convex.get_all_rewards('0x704617048F435cB679252E24148638211fb4457D', X3CRV_ETH, 17499865, ETHEREUM, web3)
...
def test_get_locked():
    locked = Convex.get_locked('0x99e703dA6A29f68a603724BAc8B68d26d235ebf6', 17499865, ETHEREUM,
                            web3, reward=False, decimals=False)
    assert locked == [[CVX_ETH, Decimal('2269137508655082138108')]]


def test_get_staked():
    staked = Convex.get_staked('0xFDB3E523fc6F0D93fce8e57e282c503c5384d08F', 17499865, ETHEREUM,
                          web3, reward=False, decimals=False)
    assert staked == [[CVX_ETH, Decimal('656399329913009873936')]]


def test_underlying():
    underlying = Convex.underlying('0x704617048F435cB679252E24148638211fb4457D', X3CRV_ETH, 17499865, ETHEREUM,
...
def test_pool_balances():
    balances = Convex.pool_balances(X3CRV_ETH, 16993460, ETHEREUM,
                             web3, decimals=False)
    assert balances == [[DAI_ETH, Decimal('165857824629254122209119338')],
