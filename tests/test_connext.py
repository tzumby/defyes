...
def test_underlying():
    ...
    next_usdc, usdc = Connext.underlying(WALLET_969, CUSDCLP, block, Chain.XDAI, web3=node)
    assert next_usdc == [GnosisTokenAddr.nextUSDC, Decimal("1064967.162791")]
    assert usdc == [GnosisTokenAddr.USDC, Decimal("1439222.771596")]

    next_weth, weth = Connext.underlying(WALLET_e6f, CWETHLP, block, Chain.XDAI, web3=node)
...
def test_unwrap():
    ...
    usdc = Connext.unwrap(2496314.966980158115136554, CUSDCLP, block, Chain.XDAI, web3=node)
