...
from defi_protocols.functions import block_to_date, date_to_block, get_logs_web3, get_node, to_token_amount
...
class PoolToken(PoolToken):
    ...
    def main_token(self) -> str:
        return self.underlying_asset_address
    ...
    def get_token_addr(self, scaling_factor):
...
class GaugeFactory(GaugeFactory):
    default_addresses: dict[str, str] = {
        ...
        "polygon": "0x22625eEDd92c81a219A83e1dc48f88d54786B017",
        # 'arbitrum': '0xb08E16cFc07C684dAA2f93C70323BAdb2A6CBFd2', # DEPRECATED
        "arbitrum": "0x6817149cb753BF529565B4D023d7507eD2ff4Bc0",
        # 'xdai': '0x809B79b53F18E9bc08A961ED4678B901aC93213a', # DEPRECATED
        "xdai": "0x83E443EF4f9963C77bd860f94500075556668cb8",
    }
    ...
    def _gauge_address(self, lp_address: str) -> str:
        if self.address == self.default_addresses["ethereum"]:
...
class Gauge(Gauge):
    ...
    def get_bal_rewards(self, wallet: str, decimals: bool = True) -> dict:
        rewards = {}
        if self.address != ZERO_ADDRESS:
            wallet = Web3.to_checksum_address(wallet)
            reward_decimals = Decimal(10**18) if decimals else 1
            rewards = {self.BAL_ADDRS[self.blockchain]: self.claimable_tokens(wallet) / reward_decimals}
...
class VebalFeeDistributor(VebalFeeDistributor):
    default_addresses: dict[str, str] = {"ethereum": "0xD3cf852898b21fc233251427c2DC93d3d604F3BB"}
...
def unwrap(blockchain: str, lp_address: str, amount: Decimal, block: int | str, decimals: bool = True) -> None:
    ...
    for n, (token_addr, balance) in enumerate(pool_tokens):
        ...
        if token.rate is not None:
            ...
            token_addr = token.get_token_addr(scaling_factor)
...
def underlying(
    ...
    for lp_address in lp_addrs:
        ...
        pool_balance_fraction = lp.balance_of(wallet) / Decimal(lp.supply)
        pool_staked_fraction = gauge.balance_of(wallet) / Decimal(lp.total_supply)

        if blockchain == "ethereum":
            vebal = Vebal(blockchain, block)
            pool_locked_fraction = vebal.balance_of(wallet, lp.address) / Decimal(lp.total_supply)
        else:
        ...
        for addr, amount in balances.items():
            ...
            if aura_staked is None:
                ...
                aura_pool_fraction = aura_staked / Decimal(lp.total_supply)
