...
class EthDerivative:
    ...
    def unwrap(self, amount: Decimal | int | float, block: int | str, web3: Web3) -> Decimal:
        ...
        if self.addr == '0xE95A203B1a91a908F9B9CE46459d101078c2c3cb' and block < 16476340:
            contract = web3.eth.contract(address=self.addr, abi='[{"inputs":[],"name":"ratio","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"}]')
            eth_value = int(Decimal(amount*(10**18))/Decimal(contract.functions.ratio().call(block_identifier=block)))
        else:
