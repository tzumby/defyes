...
def underlying(wallet, lptoken_address, block, blockchain, web3=None, execution=1, index=0, reward=False, decimals=True,
    ...
    try:
        ...
        return underlying(wallet, lptoken_address, block, blockchain, convex_staked=convex_staked,
                          gauge_address=gauge_address, reward=reward, decimals=decimals, index=index,
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def underlying_amount(lptoken_amount, lptoken_address, block, blockchain, web3=None, execution=1, index=0,
                      decimals=True):
    # If the number of executions is greater than the MAX_EXECUTIONS variable -> returns None and halts
    if execution > MAX_EXECUTIONS:
        return None
    ...
    try:
        ...
        return underlying_amount(lptoken_amount, lptoken_address, block, blockchain, decimals=decimals, index=index,
                                 execution=execution + 1)
        ...
        traceback.print_exc()
        return underlying_amount(lptoken_amount, lptoken_address, block, blockchain, decimals=decimals, index=index + 1,
                                 execution=execution)
...
def pool_balances(lptoken_address, block, blockchain, web3=None, execution=1, index=0, decimals=True, meta=False):
    ...
    try:
        ...
        return pool_balances(lptoken_address, block, blockchain, meta=meta, decimals=decimals, index=index,
                             execution=execution + 1)
