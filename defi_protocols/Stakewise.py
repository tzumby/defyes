...
from defi_protocols.functions import get_node, get_contract, to_token_amount, balance_of
...
Pools = {
    'xdai':{
        'curve':[
            {
                'name': 'sGNO-GNO',
                'LPtoken': '0xBdF4488Dcf7165788D438b62B4C8A333879B7078'
            },
            {
                'name': 'rGNO-sGNO',
...
def check_curve_pools(wallet: str, block: int | str, blockchain: str, web3: Web3 = None, decimals: bool = True, reward: bool = False):
    ...
    for item in Pools['xdai']['curve']:
        ...
        if reward:
            ...
            for element in rewards:
                result[item['name']]['rewards'].append(
                    {
                        'token': element[0],
                        'balance': element[1]
                    }
...
def check_uniswap_v3_pools(wallet: str, block: int | str, blockchain: str, web3: Web3 = None, decimals: bool = True, reward: bool = False):
...
def get_all_rewards(wallet: str, block: int | str, blockchain: str, web3: Web3 = None, decimals: bool = True, pools: bool = False)->dict:
    ...
    response = requests.get(
        f"https://api.stakewise.io/distributor-claims/{wallet}/?network={chain}"
    )
                ...
                if element['reward_token'] in [entry['token'] for entry in result['rewards']]:
                    for entry in result['rewards']:
                        if element['reward_token'] == entry['token']:
                            entry['balance'] += to_token_amount(element['reward_token'], int(element['value']), blockchain, web3=web3,
                                                   decimals=decimals)
                else:
                    result['rewards'].append(
                    {
                        'token': element['reward_token'],
                        'balance': to_token_amount(element['reward_token'], int(element['value']), blockchain, web3=web3,
                                                   decimals=decimals)
                    })
...
def underlying(wallet: str, block: int | str, blockchain: str, decimals: bool = True, web3: object = None, reward: bool = False, pools: bool = False) -> dict:
    """
    ...
    if reward:
        if not isinstance(block, str):
            raise ValueError("Historical unclaimed rewards not supported. Use block=latest to get current unclaimed rewards.")
    ...
    result = {
        ...
        "balances": [
            {
                'token': tokens[blockchain]['staking'],
                'balance': staking_balance
            },
            {
                'token': tokens[blockchain]['rewards'],
    ...
    if pools:
        if blockchain == XDAI:
            result['Curve pools'] = check_curve_pools(wallet, block, blockchain, web3=web3, decimals=decimals, reward=reward)
        if blockchain == ETHEREUM:
            result['Uniswap V3 pools'] = check_uniswap_v3_pools(wallet, block, blockchain, web3=web3, decimals=decimals, reward=reward)
