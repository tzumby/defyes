...
def underlying_all(wallet: str, blockchain: str, decimals: bool = True) -> Dict(str,List[Dict]):
    api_result = get_api_results(wallet, blockchain)
    result = {'protocol':'HiddenHand',
                        'block':'latest',
                        'positions':[]
                    }
    for entry in api_result:
        position = next((p for p in result['positions'] if p['position ID'] == entry['protocol']), None)
        
        if not position:
            position = {
                'position ID': entry['protocol'],
                'balances': []
            }
