...
from defi_protocols.Curve import Curve
...
def get_pool_info(lptoken_address, block):
    ...
    :return:
    """
    with open(str(Path(os.path.abspath(__file__)).resolve().parents[1]) + '/Convex/Convex_db.json', 'r') as db_file:
...
def update_db():
    ...
    if pools_delta > 0:
        ...
        with open(str(Path(os.path.abspath(__file__)).resolve().parents[1]) + '/Convex/Convex_db.json', 'w') as db_file:
