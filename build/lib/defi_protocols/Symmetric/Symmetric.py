...
def get_pool_info(web3, lptoken_address, block, blockchain):
    ...
    """
    result = {}
    with open(str(Path(os.path.abspath(__file__)).resolve().parents[1])+'/Symmetric/Symmetric_db.json', 'r') as db_file:
...
def update_db():
    ...
    if update == True:
        with open(str(Path(os.path.abspath(__file__)).resolve().parents[1])+'/Symmetric/Symmetric_db.json', 'w') as db_file:
