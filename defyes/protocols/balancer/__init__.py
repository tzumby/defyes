...
from defyes.protocols import ProtocolData
...
def get_protocol_data_for(
    ...
    pd = ProtocolData("Balancer", blockchain, block, "liquidity_pool_address")
    for lp_address in lp_addrs:
        ...
        if lp.balance_of(wallet):
            pool_liquidity_fraction = lp.balance_of(wallet) / Decimal(lp.supply)
            pd.add_holding(
                key=lp_address, position_type="liquidity", addr=lp_address, balance=lp.calc_balance(wallet, decimals)
            )
        ...
        for gauge_addr in gauge_addresses:
            ...
            pd.add_holding(
                ...
                addr=lp_address,
                balance=lp_balance_staked / Decimal(10**gauge.decimals if decimals else 1),
                extra_level=gauge_addr,
                extra_level_type="gauge_address",
            )
        ...
        if blockchain == Chain.ETHEREUM:
            ...
            pd.add_holding(
                ...
                addr=lp_address,
                balance=vebal.calc_balance(wallet, lp_address, decimals),
            )

        # underlyings
        balances = pool_balances(blockchain, lp_address, block, decimals)
        for addr, amount in balances.items():
            balance = amount * pool_liquidity_fraction
            pd.add_underlying(key=lp_address, position_type="liquidity", addr=addr, balance=balance)

            for gauge_addr, fraction in pool_staked_fractions:
                balance_staked = amount * fraction
                pd.add_underlying(
                    key=lp_address,
                    position_type="staked",
                    addr=addr,
            ...
            pd.add_underlying(key=lp_address, position_type="locked", addr=addr, balance=balance_locked)
        ...
        if reward:
            ...
            if blockchain == Chain.ETHEREUM:
                ...
                for addr, reward_balance in vebal_rewards.items():
                    pd.add_reward(key=lp_address, position_type="locked", addr=addr, balance=reward_balance)
    return pd.as_dict
